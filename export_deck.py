...
def generate_deck_csv(deck_60: list[int], out_path: Path) -> None:
    """Generates the deck.csv file (Kaggle/PTCG format: 1 Card ID per line, 60 lines)."""
    lines = [str(cid) for cid in deck_60]
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    logger.info("✓ deck.csv generated successfully: %s (%d cards)", out_path, len(deck_60))
...
def main():
    parser = argparse.ArgumentParser(description="Generates deck.csv and deck.svg from deck_builder.safetensors")
    parser.add_argument("--safetensors", "--ckpt", default=None, help="Path to .safetensors file")
    parser.add_argument("--csv", default="deck.csv", help="Output deck.csv path")
    parser.add_argument("--svg", default="deck.svg", help="Output deck.svg path (optional, 'none' to disable)")
    ...
    cfg = Config()
    logger.info("Configuration loaded: num_card_ids=%d, latent_dim=%d", cfg.model.num_card_ids, cfg.model.latent_dim)
