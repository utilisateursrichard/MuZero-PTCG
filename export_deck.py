#!/usr/bin/env python3
"""
export_deck.py / export_deck_svg.py
===================================
Script pour charger 'deck_builder.safetensors' (ou un checkpoint),
échantillonner un deck de 60 cartes Pokémon TCG, et l'exporter aux formats :
  - deck.csv : format officiel 60 lignes avec 1 Card ID par ligne.
  - deck.svg : visuel SVG moderne optionnel.

Usage :
    python export_deck.py --safetensors /chemin/vers/deck_builder.safetensors --csv deck.csv --svg deck.svg
"""
...
logger = logging.getLogger("export_deck")
...
def find_card_csv() -> Path:
    ...
    for c in candidates:
        if c.exists():
            return c
    raise FileNotFoundError("EN_Card_Data.csv introuvable dans le workspace.")


def generate_deck_csv(deck_60: list[int], out_path: Path) -> None:
    """Génère le fichier deck.csv (format Kaggle/PTCG : 1 Card ID par ligne, 60 lignes)."""
    lines = [str(cid) for cid in deck_60]
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    logger.info("Deck CSV exporté avec succès dans %s (%d cartes)", out_path, len(deck_60))


def generate_deck_svg(cards_info: list[dict], out_path: Path) -> None:
    """Génère un fichier SVG représentant le deck de 60 cartes."""
    ...
    sections = [
        ...
        ("Dresseurs (Trainers)", trainers, "#3182ce"),
        ("Énergies", energies, "#d69e2e"),
    ]
    if others:
        sections.append(("Autres", others, "#805ad5"))
    ...
    svg_parts.append(f'<text x="40" y="58" class="header-title">⚡ Deck PTCG MuZero</text>')
    svg_parts.append(f'<text x="{width - 40}" y="56" class="header-sub" text-anchor="end">Total cartes : <tspan font-weight="bold" fill="#38bdf8">{total_card_count}</tspan> / 60</text>')
    ...
    logger.info("Deck SVG exporté avec succès dans %s", out_path)


def main():
    parser = argparse.ArgumentParser(description="Génère deck.csv et deck.svg depuis deckbuilder.safetensors")
    parser.add_argument("--safetensors", "--ckpt", default=None, help="Chemin du fichier .safetensors")
    parser.add_argument("--csv", default="deck.csv", help="Chemin du fichier deck.csv de sortie")
    parser.add_argument("--svg", default="deck.svg", help="Chemin du fichier deck.svg de sortie (optionnel, 'none' pour désactiver)")
    parser.add_argument("--seed", type=int, default=42, help="Graine aléatoire pour le tirage")
    args = parser.parse_args()

    csv_path = find_card_csv()
    logger.info("Chargement des cartes depuis %s...", csv_path)
    ...
    if args.safetensors and Path(args.safetensors).exists():
        logger.info("Chargement des poids depuis %s...", args.safetensors)
        ...
        logger.info("Aucun poids fourni (ou introuvable). Utilisation de paramètres initialisés aléatoirement.")
    ...
    if args.csv and args.csv.lower() != "none":
        generate_deck_csv(deck_60, Path(args.csv))

    # 2. Export SVG (optionnel)
    if args.svg and args.svg.lower() != "none":
        ...
        for cid, count in counts.most_common():
            ...
            cards_info.append({
                "id": cid,
                "count": count,
                "name": info.get("name", f"Carte #{cid}"),
                "category": category,
        ...
        generate_deck_svg(cards_info, Path(args.svg))
