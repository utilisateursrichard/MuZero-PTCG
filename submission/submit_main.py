...
def agent(obs_dict) -> list[int]:
    ...
    raw_options = obs.select.option if hasattr(obs.select, "option") else (getattr(obs.select, "options", []) or [])
    min_cnt = int(getattr(obs.select, "minCount", 1)) if hasattr(obs.select, "minCount") else 1
    max_cnt = int(getattr(obs.select, "maxCount", 1)) if hasattr(obs.select, "maxCount") else 1
