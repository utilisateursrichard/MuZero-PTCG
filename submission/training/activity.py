...
class ActivityTracker:
    def __init__(self):
        ...
        self.policy_entropy_norm = 0.0
        self.policy_p_max = 0.0
        self.policy_entropy_net = 0.0
        self.policy_entropy_mcts = 0.0
        self.value_sat_pct = 0.0

        # Action distribution tracking (fenêtre glissante des 1 000 dernières actions jouées)
    ...
    @property
    def total_actions_tracked(self) -> int:
        return len(self.recent_actions)

    def record_action(self, opt_type: int) -> None:
        """Enregistre le type d'option jouée dans la fenêtre glissante récente."""
        # OptionType: ATTACK=13, ATTACH=8, PLAY=7, END=14, RETREAT=12, EVOLVE=9
        if opt_type == 13:
            name = "attack"
        elif opt_type == 8:
            name = "attach"
    ...
    def get_action_percentages(self) -> dict:
        """Retourne les pourcentages récents (fenêtre glissante des dernières actions)."""
        tot = len(self.recent_actions)
        if tot == 0:
            return {"attack": 0.0, "attach": 0.0, "play": 0.0, "end": 0.0, "retreat": 0.0, "evolve": 0.0, "other": 0.0}
        from collections import Counter
        counts = Counter(self.recent_actions)
        keys = ["attack", "attach", "play", "end", "retreat", "evolve", "other"]
        return {k: round(100.0 * counts.get(k, 0) / tot, 1) for k in keys}
    ...
    def update(
        ...
        h_status=None,
        h_grad_scale=None,
        policy_entropy_norm=None,
        policy_p_max=None,
        policy_entropy_net=None,
        policy_entropy_mcts=None,
        value_sat_pct=None,
    ):
        ...
        if h_grad_scale is not None:
            self.h_grad_scale = h_grad_scale
        if policy_entropy_norm is not None:
            self.policy_entropy_norm = policy_entropy_norm
        if policy_p_max is not None:
            self.policy_p_max = policy_p_max
        if policy_entropy_net is not None:
            self.policy_entropy_net = policy_entropy_net
        if policy_entropy_mcts is not None:
            self.policy_entropy_mcts = policy_entropy_mcts
        if value_sat_pct is not None:
            self.value_sat_pct = value_sat_pct
        self.last_activity_time = time.time()
